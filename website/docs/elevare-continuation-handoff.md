...
| Concierge | Human-operated cases, shortlists of 1–3 actual candidates, professional interest/decline/clarification, client selection and consented introductions. Direct profile inquiries remain separate. Rematches preserve history. The September 11 notification worker adds Resend delivery, owner preferences, optional reminders, and plain admin alerts to `mlitfin@elevarefit.org`. Historical disabled events are not replayed. See `marketplace-notification-delivery.md` and the notification integration/browser reports. |
