...
export const metadata = buildMetadata({
  ...
  description: "Get a one-time StageLab competition-prep or physique assessment for $0.99. No account or subscription required, and submitted photos are not retained.",
...
const faqs = [
  ...
  { question: "Do I need a StageLab account?", answer: "No. The $0.99 purchase provides one website analysis and does not create an account, subscription, app credit, or mobile entitlement." },
...
const structuredData = [
  {
    ...
    offers: { "@type": "Offer", price: "0.99", priceCurrency: "USD", availability: "https://schema.org/InStock", url: absoluteUrl("/stagelab/quick-analysis/") },
  },
  {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    ...
    <div className="container">
      ...
      <section className="hero quick-analysis-hero">
        <div>
          ...
          <p>Get a one-time StageLab AI physique analysis for $0.99 using 3-5 current photos. Choose a competition-prep assessment or a comparison with competition-level conditioning.</p>
          <div className="quick-analysis-badges" aria-label="Product details">
            <span>$0.99 one time</span><span>No account required</span><span>Photos never stored by ElevareFit</span>
          </div>
        </div>
      ...
      <section className="section quick-analysis-intro-grid">
        ...
        <article className="panel"><span className="stat-label">02 · Secure checkout</span><h2>Pay once through Stripe.</h2><p>The $0.99 USD payment is non-recurring and covers one successfully completed website analysis.</p></article>
      </section>
      ...
      <section className="section" aria-labelledby="quick-analysis-faqs">
        ...
        <div className="tool-faq-grid">{faqs.map((faq) => <article className="tool-faq-card panel" key={faq.question}><h3>{faq.question}</h3><p>{faq.answer}</p></article>)}</div>
      </section>
    </div>
  );
}
