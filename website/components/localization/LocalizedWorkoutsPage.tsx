...
async function WorkoutDetail({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  return (
    <div className="container">
      ...
      {days.length ? <><section className="section"><article className="panel training-directory-card"><div className="section-head"><div className="eyebrow">{messages.detail.tableEyebrow}</div><h2 className="section-title">{messages.detail.tableTitle}</h2><p className="section-copy">{messages.detail.tableCopy}</p></div><div className="training-day-stack">{days.map((day) => <div key={day.label} className="training-day-block"><div className="training-day-head"><h3>{localizeWorkoutDayLabel(day.label, locale)}</h3><span>{formatNumber(day.exercises.length, locale)} {day.exercises.length === 1 ? messages.detail.exerciseSingular : messages.detail.exercisePlural}</span></div><div className="training-table-wrap"><table className="training-table"><thead><tr><th>{messages.detail.exercise}</th><th>{messages.detail.section}</th><th>{messages.detail.sets}</th><th>{messages.detail.reps}</th><th>{messages.detail.rest}</th><th>{messages.detail.notes}</th></tr></thead><tbody>{day.exercises.map((entry) => <tr key={entry.id}><td>{entry.exercise ? <TrackedLink className="blog-link" href={localizePathname(`/exercises/${entry.exercise.slug}`, locale)} eventName="exercise_open" eventParams={{ exercise_slug: entry.exercise.slug, source_page: `localized_workout_${slug}_table` }}>{localizeExerciseName(entry.exercise.name, locale)}</TrackedLink> : localizeExerciseName(entry.exerciseName, locale)}</td><td>{localizedSection(entry.section, locale)}</td><td>{entry.sets ?? messages.detail.asWritten}</td><td>{entry.reps ?? messages.detail.asWritten}</td><td>{localizedRest(entry.restSeconds, locale)}</td><td>{messages.detail.followTargets}</td></tr>)}</tbody></table></div></div>)}</div></article></section><TrainingLogbookCta title={messages.cta.detailTitle} description={messages.cta.detailCopy} ctaContext={`localized_workout_${slug}`} locale={locale} /></> : null}
