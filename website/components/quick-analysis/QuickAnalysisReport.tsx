...
function ResultList({ items }: { items: string[] }) {
  return items.length ? <ul>{items.map((item, index) => <li key={`${index}-${item}`}>{item}</li>)}</ul> : <p>No additional items were identified.</p>;
}
...
function ResultLimitations({ result, mode }: { result: QuickAnalysisResult; mode: QuickAnalysisMode }) {
  return (
    <section className="quick-analysis-limitations panel">
      <div><span className="stat-label">Limitations</span><h2>What can affect this visual estimate</h2><ResultList items={result.limitations} /></div>
  ...
  return (
    <>
      ...
      <section className="quick-analysis-report-grid">
        ...
        <article className="panel"><span className="stat-label">Presentation</span><h2>Presentation</h2><p>{result.presentation_assessment}</p></article>
      </section>
      ...
      <section className="panel"><span className="stat-label">Judge-style perspective</span><h2>Snapshot perspective</h2><p>{result.judges_perspective}</p></section>
...
function PhysiqueCheckReport({ result }: { result: QuickAnalysisResult }) {
  ...
  const scoreCards = [
    ...
    ["Presentation", result.presentation_score, result.presentation_assessment],
  ] as const;
    ...
    <>
      <section className="quick-analysis-report-hero quick-analysis-readiness-hero panel">
        <div>
          ...
          <p>{result.summary}</p>
          <p className="fine-print">Stage Readiness reflects how closely the visible physique aligns with typical competition-level conditioning, muscularity, symmetry, and presentation markers. It does not predict contest placement.</p>
        </div>
        <div className="quick-analysis-condition-score"><span>Stage Readiness</span><strong>{result.stage_readiness_score}/100</strong><small>{result.stage_readiness_category}</small></div>
      </section>

      <section className="quick-analysis-snapshot-grid">
        <article className="panel"><span className="stat-label">Visual distance from stage condition</span><strong className="quick-analysis-snapshot-value">{distance}</strong></article>
      ...
      <section className="quick-analysis-report-grid">
        {scoreCards.map(([label, score, assessment]) => (
          <article className="panel quick-analysis-score-card" key={label}>
            <div><span className="stat-label">{label}</span><strong>{score}/100</strong></div>
            <p>{assessment}</p>
            {label === "Conditioning" ? <ResultList items={result.visible_conditioning_markers} /> : null}
          </article>
        ))}
      </section>
      <p className="quick-analysis-weighting-note fine-print">Stage Readiness weighting: conditioning 40%, muscularity 25%, symmetry and proportions 20%, presentation 15%.</p>

      <section className="quick-analysis-report-grid">
        <article className="panel"><span className="stat-label">Visible strengths</span><h2>What already looks stage-oriented</h2><ResultList items={result.visible_strengths} /></article>
        <article className="panel"><span className="stat-label">Visible differences</span><h2>What separates the current look from stage condition</h2><ResultList items={result.areas_to_improve} /></article>
      </section>
      ...
      <section className="panel"><span className="stat-label">Judge-style read</span><h2>Current visual perspective</h2><p>{result.judges_perspective}</p></section>
